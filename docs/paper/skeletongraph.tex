...
%
% TITLE: "FRONTIER Coding Agents Don't Have a Retrieval Problem". The word
% "frontier" is LOAD-BEARING — never drop it. Unqualified, the title is refuted
% by our own Table tab:react, where retrieval is worth +7pp on the open-weight
% model (fusion 42.0% vs closed-book 35.0%). The claim holds only in the
% memorization-saturated frontier deployment, and the "On the title" paragraph
% after the contributions states the counter-example explicitly so a reviewer
...
%      cues shows retrieval is ENTITY-ANCHORED — strip the symbols and SG's own
%      recall drops 86% -> 55% (within-task 0.125, 95% CI [0.010,0.260], n=26 —
%      SIGNIFICANT, and this is the leg the ceiling rests on).
%      WHAT WE DO *NOT* CLAIM (tested 2026-07-31, do not re-add): that SG's MARGIN
%      over the baseline collapses to parity. That second-order effect is +0.058
%      within-task, 95% CI [-0.093,0.215], P(>0)=0.77, and only 5 of 26 tasks show
%      it. Needs n~370 to resolve; not affordable and NOT NEEDED — the ceiling
%      argument only requires that running all three paradigms fails on
%      symptom-only issues, which the first-order drop establishes alone.
%      Never write "collapses to parity" / "indistinguishable from lexical".
%      RECALL CONVENTION (decided 2026-07-31, do not re-mix): tasks where the
%      agent never invoked SG are EXCLUDED from recall averages everywhere —
%      adoption events, not retrieval failures. This is the rule that makes the
%      headline 86.2% not 83.6%, and it MUST also apply to the ceiling grid.
%      Scoring them 0.0 instead makes the prose cell read -0.006 ("parity");
%      that is the OTHER convention and mixing the two is how the earlier draft
%      claimed parity while the headline used exclusion. _rec1() in
%      make_paper_figures.py returns None (not 0.0) to enforce this.
%      The lexical baseline falls in parallel, so
...
%      better) and in BOTH prose cells its CUMULATIVE recall OVERTAKES SG's
%      (SWE .964 vs .946; rebench .715 vs .632). => A retrieval layer delivers FILES, not knowledge of
...
\title{Frontier Coding Agents Don't Have a Retrieval Problem:\\
...
% Fixed, not \today: a preprint's date must not change every time it recompiles.
% Bump manually when a new version is actually posted.
\date{August 2026}
...
the comparison on a decontaminated benchmark of unseen repositories. Retrieval turns
out to be anchored to those symbols: \sg{}'s first-search file recall falls from
86\% to 55\%, a within-task degradation of $0.125$ ($95\%$ CI $[0.010, 0.260]$), and
plain text search falls with it. Issuing more searches does not recover the
...
\item \textbf{A localization ceiling, and evidence it is categorical.} Removing the
code symbols from an issue degrades our three-paradigm retriever sharply and
significantly---$0.125$ within task, $95\%$ CI $[0.010, 0.260]$---and additional
searches do not recover it. Running lexical, dense and structural retrieval
simultaneously therefore does not solve symptom-only localization, which locates the
limit at similarity-based localization rather than at our implementation and
identifies the open problem as reasoning over a representation of program
behavior---not a better index (Section~\ref{sec:ceiling}). Crossing this factor with
whether the model has memorized the repository yields the sharpest form of the
decoupling: across all four conditions, including a decontaminated benchmark and the
conditions where our retrieval margin is a few points of recall, the cost saving
persists at $-21\%$ to $-31\%$. The unifying account is that both factors
...
\paragraph{On the title, and the word ``frontier'' in it.}
The title is a claim about a specific regime, and the qualifier is load-bearing. In
the frontier-agent deployment the model has substantially memorized these
repositories, so retrieval quality cannot express itself in outcomes: solve rate is a
tie, and the median task is no cheaper despite first-search file recall rising from
66\% to 86\% and function-level localization from 0\% to roughly 80\%. That is the
sense in which retrieval is not the binding constraint there.

It is emphatically \emph{not} a claim that retrieval never matters, and this paper
contains the counter-example. On the open-weight model of Section~\ref{sec:systems},
which memorizes far less, a control receiving no repository code solves 35.0\% of
tasks and structural retrieval reaches 42.0\%---a genuine seven-point effect
(Table~\ref{tab:react}). Retrieval is worth little precisely where the model already
knows the code and a great deal where it does not. Readers who take only one sentence
from the title should take that one. What we establish is stated precisely in
Section~\ref{sec:ceiling}, with the observation that would falsify it, and the limits
of its generality in Section~\ref{sec:threats}.
...
elaborate agentic scaffolding, \citet{grepallyouneed} find text search generally
outperforming vector retrieval inside several production agent CLIs, and our own
results show lexical navigation reaching the gold file in the large majority of
tasks. \emph{Implied cost:} cheap per call, but file-granular --- the agent must
read to narrow further, and those reads are the dominant token sink we measure.
...
The general lesson---that the harness can matter as much as the retriever---has
independent support. \citet{grepallyouneed} compare text and vector retrieval
across a custom loop and three production agent CLIs and find that accuracy
depends strongly on which harness and tool-calling style is used even when the
underlying data are held fixed. Our exclusion result is an extreme instance of
that effect: a harness property decided whether a retriever participated at all.
It is also why we never pool numbers across our two harnesses, and why we treat
the deployment magnitudes in Section~\ref{sec:decoupling} as specific to the
agent we measured rather than as properties of retrieval in general.
...
Figure~\ref{fig:ceiling} shows the result, and the first-order finding is the one
the argument rests on: running all three paradigms at once does not survive the
removal of the symbols. \sg{}'s first-search recall falls from 86\% to 55\% as the
cues are withdrawn. Measured within task---the same issues under both
conditions---that degradation is $0.125$ on average, with a bootstrap 95\%
confidence interval of $[0.010,\,0.260]$ over the 26 paired tasks across both
benchmarks. The built-in lexical baseline degrades as well, from 66\% to 49\%.
Neither similarity over tokens, nor similarity in an embedding space, nor
adjacency in a call graph withstands the loss of the symbol names, and running
all three together does not change that.

We are deliberately more careful about the second-order question---whether
\sg{}'s \emph{margin} over the baseline shrinks faster than the baseline itself
does. The point estimates suggest it: the margin is $+0.200$ when the issue names
the code and $+0.065$ when it does not, and on the decontaminated benchmark
$+0.160$ against $+0.056$. But tested within task that difference is
$+0.058$ with a 95\% interval of $[-0.093,\,0.215]$, and only 5 of 26 tasks
individually show it. At this sample size we can establish that the retriever
degrades sharply; we cannot establish that it degrades \emph{to parity} with
lexical search, and we do not claim it. The ceiling argument does not require
that stronger statement---what matters is that combining every non-LLM paradigm
available leaves symptom-only localization unsolved, which the first-order result
shows on its own.
...
Table~\ref{tab:grid} gives the four cells numerically. Read the recall columns
first: both systems lose accuracy as the cues are withdrawn, \sg{} from $0.861$ to
$0.549$ and the baseline from $0.661$ to $0.493$. The margin between them is
smaller in the prose conditions than in the original ones, in the same direction on
both benchmarks, though as noted above that second-order difference is not resolved
at this sample size. The cost column behaves differently, and this is the
comparison that carries the section: it stays between $-21\%$ and $-31\%$ in every
cell, including the two where the retrieval margin has shrunk to a few points of
recall. Whatever the exact size of the remaining retrieval advantage, it is not
what the cost saving is tracking.
...
(memorized)    & prose-only    & $0.696 \to 0.762$ & $\mathbf{+0.065}$ & $-21.1\%$ & $-22.6\%$ \\
\addlinespace
SWE-rebench    & original      & $0.577 \to 0.737$ & $+0.160$          & $-26.4\%$ & $-35.6\%$ \\
(unseen)       & prose-only    & $0.493 \to 0.549$ & $\mathbf{+0.056}$ & $-31.3\%$ & $-29.3\%$ \\
\bottomrule
\end{tabular}
\caption{The decoupling, by condition. Retrieval accuracy degrades as location cues
are withdrawn; the cost saving does not. All cells $n=15$, paired, test-execution
verified. The $\Delta$ column is a point estimate: at this sample size the
difference between the original and prose-only margins is not statistically
resolved (see text). Solve rate is a tie or $\pm1$ in every cell and is omitted.}
\label{tab:grid}
\end{table}

Four points of care belong with this table. The SWE-Verified rows are restricted to
...
not that it trends. In the rebench prose-only cell one baseline task terminated in
error and is excluded, leaving 14 baseline runs against 15. Finally, recall here uses
the same rule as Table~\ref{tab:retrieval}: tasks on which the agent never invoked
\sg{} are excluded from the recall average, because they record an adoption decision
rather than a retrieval outcome. One, two and three tasks are excluded in the three
prose and unseen cells respectively; cost and turn figures use every paired task, since
adoption does not change what a run was billed. The convention matters more here than
at $n=100$ --- scored the other way the prose-only cell reads $-0.006$ instead of
$+0.065$ --- which is itself a reason to treat these cells as directional and to
prefer the larger sample for any quantitative claim.
...
does not track retrieval quality.} In the prose-only condition on the memorized
benchmark, where \sg{} retrieves barely better than the baseline, cost still falls
by 21\%. The decontaminated benchmark makes the same point more sharply: its
largest cost saving ($-31.3\%$) occurs in the cell with its \emph{smallest}
retrieval margin ($+0.056$), and its smallest saving in a cell with a larger one.
Whatever \sg{} is doing to reduce cost, it is not out-retrieving the baseline; it
is bounding how far the agent wanders before it commits. This is the one claim in
the section that does not depend on resolving the retrieval margin precisely---the
cost effect is present and large in cells where that margin is, on any reading,
small.
...
both, $0.762$ with memorization alone, $0.737$ with cues alone, $0.549$ with
...
repository's actual vocabulary, and greps better, lifting recall by $+0.128$ to
$+0.268$ over its first search. In both prose-only conditions this is decisive
enough to overturn the ranking: the baseline's cumulative recall finishes at
$0.964$ against \sg{}'s $0.946$ on the memorized benchmark, and at $0.715$ against
$0.632$ on the unseen one. Given enough turns, the agent that explored the
repository ended up locating the code \emph{better} than the agent handed a ranked
list. \sg{} gets there in half the searches and for less money; it does not get
further.
...
We note that \citet{grepallyouneed}, comparing retrieval strategies across three
production agent CLIs, report substantial harness-dependent variation in outcomes,
which is a reason to expect the \emph{size} of our effect to move across agents
even where its direction does not.
