...
\begin{abstract}
Structural code retrieval---handing a coding agent the exact functions it needs
instead of letting it read its way to them---is a natural way to cut what the agent
spends, and an active line of research and product work pursues it. \sg{} runs
lexical, dense-semantic, and call-graph retrieval simultaneously, returns whole
functions, and serves them over the Model Context Protocol. We measure what that
buys inside a real agent: Claude Code in headless deployment, 100 SWE-bench Verified
tasks, every candidate fix adjudicated by executing the project's own test suite.

Retrieval quality improves sharply. The first search returns the correct \emph{file}
for 86\% of tasks against 66\% for the agent's built-in text search, and names the
correct \emph{function} in roughly 80\% against 0\%---text search matches lines and
returns files, so function-level localization is not something it does poorly but
something it cannot do. Cost does not follow. The median task is no cheaper
($+1.9\%$); the entire saving sits in the expensive tail ($-42.5\%$ at the 95th
percentile), and solve rate is a statistical tie.

Because \sg{} runs every non-LLM retrieval paradigm at once, we can ask whether the
remaining limit is our index or the category. Stripping the symbol names from the
issue text, and repeating the comparison on a decontaminated benchmark of unseen
repositories, drops its first-search recall from 86\% to 55\%, with text search
falling alongside it; issuing more searches does not recover the gap. Memorization
and location cues turn out to be one quantity---knowledge of where to look, held by
the model or supplied by the issue---and where neither is present the agent acquires
it by exploring instead. A retrieval layer supplies candidate files, not knowledge of
the repository, and the cost saving persists throughout, so what it reduces is how
far the agent wanders rather than how well it finds code. We release the harness,
task lists, and per-task records.
\end{abstract}
...
production frontier coding agent---Claude Code in headless (\texttt{-p}) mode,
invoked as \texttt{--model sonnet}, which the run transcripts record as resolving
to \texttt{claude-sonnet-5} for every task reported here---against the same agent,
same model, using only its unmodified built-in tools. This measures deployment economics under an agent we do not
...
separate arms we implement---\texttt{grep} and \texttt{bm25}---running on
\texttt{nvidia/nemotron-3-super-120b-a12b}, an open-weight mixture-of-experts model
with 120B total and 12B active parameters. Every retrieval, cost, turn and context figure in
...
\caption{Whole systems on a second model family
(\texttt{nemotron-3-super-120b-a12b}, 100 tasks; each pipeline keeps its own
retrieval interface).}
\label{fig:tokens}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Controlled retrieval ablation in the react loop:
\texttt{nemotron-3-super-120b-a12b}, 100 tasks, identical action space, only the
retrieval backend varies.}
\label{tab:react}
\end{table}
