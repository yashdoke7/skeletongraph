...
%      symbols from the issue and ~2/3 of it goes (86% -> 55% recall; margin over
%      baseline +0.200 -> +0.065 on SWE, +0.160 -> +0.056 on rebench).
...
from 86\% to 55\%, and \sg{}'s margin over plain text search---$+0.200$ with the
symbols present---falls by roughly two-thirds, to a residue this sample cannot
separate from zero. Issuing more searches does not recover the
...
55\% as the cues are withdrawn---and the lexical baseline falls with it, from
66\% to 49\%. The two curves converge: running all three paradigms at once buys
$+0.200$ recall over built-in lexical search when the issue names the code, and
$+0.065$ once it does not---a margin this sample cannot distinguish from zero.
...
be read against each other. The recall column collapses, and it does so by
almost exactly the same proportion on both benchmarks: an advantage of $+0.200$
falls to $+0.065$ when the symbols are removed from the memorized benchmark, and
$+0.160$ falls to $+0.056$ on the unseen one---roughly two-thirds of the advantage
gone in each case, leaving a residue that $n=15$ cannot separate from zero. The
cost column does not collapse with it. It stays between $-21\%$ and $-31\%$ in
every cell, including the cells where the retrieval advantage has all but
disappeared.
