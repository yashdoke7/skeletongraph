...
# Both NIM and vLLM expose /v1/chat/completions and /v1/models.
# This path is used by the eval harness for the summary-llm-* arms,
# so summaries are generated by the same 70B/32B model as the agent.


def is_openai_available(base_url: str, api_key: str = "EMPTY") -> bool:
    """Return True if an OpenAI-compatible endpoint is reachable. Never raises."""
    try:
...
def generate_summary_openai(
    ...
    """Generate a one-line summary via OpenAI-compatible /v1/chat/completions.

    Works with NIM (nvidia), vLLM, and any OpenAI-compatible endpoint.
    Same prompt/cleanup as generate_summary_ollama, different transport.
    Returns:
        (cleaned_summary, usage_dict) where usage_dict has 'prompt_tokens' and 'completion_tokens'.
    """
    if len(body) > 2000:
        body = body[:1800] + "\n    # ... (truncated)"

    user_content = _SUMMARIZE_PROMPT.format(fqn=fqn, signature=signature, body=body)
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            ...
            return cleaned or None, usage
    except Exception:
        return None, {}


def batch_generate_openai(
    items: List[Tuple[str, str, str]],   # (fqn, signature, body)
    model: str,
    base_url: str,
    api_key: str = "EMPTY",
    ...
    """Generate summaries for a batch of functions via OpenAI-compatible API.
    ...
    results: Dict[str, str] = {}
    for fqn, signature, body in items:
        summary, usage = generate_summary_openai(
            fqn=fqn, signature=signature, body=body,
